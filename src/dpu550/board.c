...
lowlevel_init(void)
{
    ...
    AT91C_BASE_PMC->PMC_MOR = ((OSC_STARTUP * SCLK / 8000000 + 1) << 8 ) |
        AT91C_CKGR_MOSCEN;
    ...
    // Set the PLL with the given divider as the master clock source
    AT91C_BASE_PMC->PMC_MCKR = AT91C_PMC_CSS_PLL_CLK | (CLK_DIV << 2);
