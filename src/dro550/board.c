...
lowlevel_init(void)
{
    ...
    // Set the flash mode to 1 microseconds with 2R/3W cycles wait states
    AT91C_BASE_EFC->EFC_FMR = ((MCLK / 1000000 + 2) << 16) | AT91C_MC_FWS_1FWS;
    ...
    save = *((int *) AT91C_ISRAM);
    *((int *) AT91C_ISRAM) = 0xdeadbeef;
    if (*((int *) 0x0) != 0xdeadbeef)
        AT91C_BASE_MC->MC_RCR = AT91C_MC_RCB;
    *((int *) AT91C_ISRAM) = save;
}

void
efc_command(uint8_t cmd, uint8_t pagen)
{
    interrupt_disable();

    // Wait for EFC ready
    while ((AT91C_BASE_EFC->EFC_FSR & AT91C_MC_FRDY) == 0);

    // Clear GBNVM bit 2
    AT91C_BASE_EFC->EFC_FCR = (0x5a << 24) | (pagen << 8) | cmd;
